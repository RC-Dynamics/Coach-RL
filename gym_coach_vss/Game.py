...
class Stats:
    ...
    def get_DDB(self, data):
        ...
        for robot in data.frame.robots_blue:
            sbd.append(np.sqrt((robot.x - ball.x)**2 + (robot.y - ball.y)**2)) # y
    ...
    def get_controll(self, history):
        ...
        for i in range(len(ball)):
            ...
            co += sum([self.is_controling(yellow[i], ball[i]) for yellow in yellows])
    ...
    def is_controling(self, robot_pos, ball_pos):
        dist = np.sqrt((robot_pos.x-ball_pos.x)**2 + (robot_pos.y-ball_pos.y)**2)
...
class History:
    def __init__ (self, MAX, num_robotsBlue=3, num_robotsYellow=3):
        ...
        self.listOfBlueRobots = [collections.deque(maxlen=MAX)] * num_robotsBlue
        self.listOfYellowRobots = [collections.deque(maxlen=MAX)] * num_robotsYellow       
